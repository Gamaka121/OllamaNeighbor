...
TOOLS = [
    ...
    _tool(
        "list_files",
        "Показывает список файлов в рабочей папке Ollama Neighbor.",
    ),

    _tool(
        "read_text_file",
        "Читает текстовый файл из рабочей папки Ollama Neighbor.",
        {
            "filename": {
                "type": "string",
    ...
    _tool(
        "create_folder",
        "Создаёт папку в рабочей директории.",
        {
            "name": {
                "type": "string",
    ...
    _tool(
        "open_file",
        "Открывает файл средствами Windows.",
        {
            "filename": {
                "type": "string",
                "description": "Имя файла.",
    ...
    _tool(
        "open_notepad",
        "Открывает Блокнот Windows. Можно сразу передать текст.",
        {
            "text": {
                "type": "string",
    ...
    _tool(
        "press_key",
        "Нажимает клавишу или сочетание клавиш. Требует подтверждения.",
        {
            "key": {
                "type": "string",
    ...
    _tool(
        "type_text",
        "Печатает текст в активное окно. Требует подтверждения.",
        {
            "text": {
                "type": "string",
...
Текущее время и текущая дата — это состояние компьютера пользователя.
...
сейчас времени?», «какая сегодня дата?», «какой сегодня день?» или
«что сейчас за дата?», ОБЯЗАТЕЛЬНО вызови get_current_datetime для этого
нового вопроса. Не используй web_search для времени компьютера.
"""
...
class NeighborApp(ctk.CTk):
    ...
    def execute_tool(
        ...
        try:
            ...
            return (
                f"Ошибка аргументов инструмента "
                f"{name}: {error}"
            )
